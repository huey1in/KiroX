...
新增
- 新增 Outlook 邮箱提供商，支持 IMAP 与 Microsoft Graph 两种收信模式
- 新增 iCloud 邮箱账号池，可在 GUI 中导入、管理并用于注册任务
- 新增独立 IP 管理页面，支持代理选择、类型与国家识别、搜索及测试结果详情
- 新增 Windows 桌面完成通知与任务结束提示音
- 新增任务、日志、提示音和浏览器指纹偏移曲线等设置项

变更
- 将独立注册页面整合为新建任务模态框，并精简概览页任务状态信息
- 重构 IP 管理流程并移除订阅功能
- 将 CLI 版的注册协议、遥测、指纹与邮箱处理能力同步到 GUI 版
- 优化注册并发调度、邮件轮询、结果写入和任务状态管理
- 重整项目图标、邮箱图标、下拉组件和任务进度展示
- 清理冗余前端代码、生成文件与本地热更新逻辑
- 更新中、英、日 README，并补充 IPWO 特别鸣谢

修复
- 修复语言切换后部分动态内容未立即刷新的问题
- 修复 Star History 图表地址
